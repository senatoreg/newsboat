...
PodboatQueueManager::PodboatQueueManager(ConfigContainer* cfg_, std::string queue_file)
	: cfg(cfg_)
	...
	, _name("podboat")
{
	ConfigPaths configpaths;
		...
		queue_file = std::move(configpaths.queue_file());
        }
}
...
extern "C" newsboat::QueueManager* plugin_init(newsboat::ConfigContainer* cfg, std::string queue_file) {
	return new newsboat::PodboatQueueManager(cfg, queue_file);
}
